...
export class ServerService {
    ...
    public sendMessage(room: String | null, type: String, content: any) {
        if (this.active && this.io != null) {
            if (room != null) {
                this.io?.to(room.toString()).emit(ServerService.messages.out.new_player), 
                // {
                //     type: this.inputMessage.find((name) => name.type === "NEW_PLAYER")?.type,
                //     content: this.inputMessage.find((name) => name.type === "NEW_PLAYER")?.do
                // }
                {
                    type: "NEW_PLAYER",
                    do: this.doNewPlayer
    ...
    private doNewPlayer(data: String) {
    console.log("New Player entered the room");
}
}